(function () {
    ...
    var Controls = function Controls() {
      ...
      this.keys = {};
      jquery(window).on('keydown', this._onKeyDown.bind(this)).on('keyup', this._onKeyUp.bind(this)).on('touchstart mousedown', this._onMouseDown.bind(this)).on('touchend mouseup', this._onMouseUp.bind(this));
    };
    ...
    jquery('.mute-btn').on('touchstart click', function () {
      if (soundMuted) {
        soundMuted = false;
    ...
    var Player = function Player(el, game) {
      this.el = el;
      this.game = game;
      this.pos = {
        x: this.game.WORLD_WIDTH / 2 - 8,
        y: this.game.WORLD_HEIGHT / 2 + 3
      };
      this.score = 0; // Contains number of jump events registered since the key was first pressed
    ...
    Player.prototype.onFrame = function (delta, hasStarted) {
      if (hasStarted) {
        ...
        this.el.css('transition', '0s'); // Check if the control keys or mouse is pressed

        if (Controls$1.isKeyPressed()) {
          ...
          if (!this.lastFrameKeyPressed) {
            ...
            this.yB4 = this.pos.y; // only play flap if sound is not muted

            if (!Controls$1.getSoundMuted()) {
              // var flap = document.getElementById('Flap');
              // flap.volume = 0.1;
              jquery('#Flap').trigger('play');
            }
          ...
          // Reset jump settings
          this.lastFrameKeyPressed = false;
          this.continuousJump = false; // Only start rotating when jump is finished and rayman starts descending
    ...
    var Wings = function Wings(el, game) {
      this.el = el;
      this.game = game; // Contains number of jump events registered since the key was first pressed
    };
    ...
    var Pipe = function Pipe(el, game, num) {
      this.el = el;
      this.game = game;
    ...
    */
    ...
    Pipe.prototype.reset = function () {
      if (this.num === 1) {
        ...
        this.pos.x = INITIAL_POSITION_X + 2 * WIDTH$1;
      }
    ...
    Pipe.prototype.checkCollisionWithPlayer = function () {
      if (this.player.pos.x + this.player.getWidth() > this.pos.x && this.player.pos.x < this.pos.x + WIDTH$1 / 2) {
    ...
    Pipe.prototype.updateScore = function () {
      if (this.pos.x + WIDTH$1 / 2 < this.player.pos.x && !pipePassed) {
        this.player.score++;
        jquery('.Score').html(this.player.score);
        pipePassed = true;
      }
    };
    ...
    var Ground = function Ground(el, game) {
      this.el = el;
      this.game = game;
      this.player = game.player;
      this.pos = {
    ...
    var Cloud = function Cloud(el, game) {
      this.el = el;
      this.game = game;
      this.player = game.player;
      this.pos = {
    ...
    var posArr$1 = [0, 5, 10, 15, 20, 25, 30, 35, 40];
    var SPEED$3 = 25; // * 10 pixels per second

    var WIDTH$2 = 16;
    var INITIAL_POSITION_X$1 = 51;
    var INITIAL_POSITION_Y$1 = posArr$1[Math.floor(Math.random() * posArr$1.length)];

    var Coin = function Coin(el, game) {
      this.el = el;
      this.game = game;
      this.player = game.player;
      this.pos = {
    ...
    Coin.prototype.reset = function () {
      this.pos.x = INITIAL_POSITION_X$1;
      this.pos.y = INITIAL_POSITION_Y$1;
      SPEED$3 = 25;
    };

    Coin.prototype.onFrame = function (delta, hasStarted) {
      if (hasStarted) {
        this.pos.x -= delta * SPEED$3;
        SPEED$3 += 0.000625;
      } // Checks if the element has passed the left side of the game screen compconstely and respawns it on the right
      // side with a random y position from posArr
      ...
      if (this.pos.x < -WIDTH$2) {
        var newY = posArr$1[Math.floor(Math.random() * posArr$1.length)];
        this.pos.y = newY;
        this.pos.x = INITIAL_POSITION_X$1;
      }
    ...
    var Game = function Game(el) {
      ...
      this.highScore = 0;
      var fontsize = Math.min(window.innerWidth / 35, window.innerHeight / 75);
      el.css('fontSize', fontsize + 'px'); // Cache a bound onFrame since we need it each frame.
    ...
    Game.prototype.onFrame = function () {
      ...
      if (Controls$1.keys.up || Controls$1.keys.space || Controls$1.keys.click) {
      ...
      this.lastFrame = now; // Update game entities.

      this.player.onFrame(delta, this.hasStarted); // this.wings.onFrame(delta, this.hasStarted);

      this.ground.onFrame(delta);
      this.cloud.onFrame(delta);
      this.pipe.onFrame(delta, this.hasStarted);
      this.pipe2.onFrame(delta, this.hasStarted);
      this.coin.onFrame(delta, this.hasStarted); // Request next frame.
    ...
    Game.prototype.start = function () {
      this.reset(); // Restart the onFrame loop
    ...
    Game.prototype.gameover = function () {
      ...
      if (!Controls$1.getSoundMuted()) {
        // var death = document.getElementById('Death');
        // death.volume = 0.4;
        jquery('#Death').trigger('play');
      } // Should be refactored into a Scoreboard class.


      var that = this;
      var scoreboardEl = this.el.find('.Scoreboard');
      scoreboardEl.addClass('is-visible').find('.Scoreboard-restart').one('touchend click', function () {
        scoreboardEl.removeClass('is-visible');
        that.start();
      });
      jquery('.Scoreboard-Score>span').html(this.player.score);

      if (this.player.score > this.highScore) {
        this.highScore = this.player.score;
      }

      jquery('.Scoreboard-Highscore>span').html(this.highScore);
    };
    /**
    ...
    jquery(function () {
      var game = new Game(jquery('.GameCanvas'));
      game.start();
    });
